...
function buildCalculatorHTML() {
  return `
    ...
    <p class="calc-subtitle">Interaktív kalkulátor a DIMOP Plusz-1.2.6/C-26 (budapesti) pályázathoz. Válaszd ki a fejlesztési célokat és eszközöket!</p>
    ...
    <!-- Scoring -->
    <h3>Értékelési szempontok <span class="calc-hint">(max. 14 pont, min. 2 kell)</span></h3>
    <div class="calc-scoring">
      ...
      <div class="calc-score-row">
        ...
        <span class="calc-score-name">Digitális érettség (bemeneti felmérő)</span>
        <span class="calc-score-how">
          <select id="calcDigMaturity" class="calc-select-sm">
            <option value="low" selected>Bemeneti pontszám 0-1 (+4 pont)</option>
            <option value="mid">Bemeneti pontszám 2-5 (0 pont)</option>
          </select>
        </span>
        <span class="calc-score-val" id="scoreDigMaturity">4p</span>
      </div>
      <div class="calc-score-row">
      ...
      <div class="calc-score-total" id="scoreTotalRow">
        ...
        <span id="scoreTotalVal" class="calc-score-total-val">0/14 pont</span>
      </div>
    </div>
...
function bindCalculatorEvents() {
  ...
  document.getElementById('calcDigMaturity').addEventListener('change', () => recalculate());
...
function recalculate() {
  ...
  const digMaturity = document.getElementById('calcDigMaturity')?.value;
  const hasDFK = document.getElementById('calcDFK')?.checked;

  const digPoints = digLevel === 'high' ? 4 : 2;
  const digMaturityPoints = digMaturity === 'low' ? 4 : 0; // C-26 8.2/7: bemeneti 0-1 pont → 4 pont, 2-5 → 0
  ...
  const totalPoints = kiberPoints + miPoints + digPoints + digMaturityPoints + dfkPoints;
  ...
  scoreEl('scoreDigMaturity', digMaturityPoints);
  ...
  if (totalValEl) {
    totalValEl.textContent = `${totalPoints}/14 pont`;
...
function getCalcState() {
  const state = {
    ...
    digMaturity: document.getElementById('calcDigMaturity')?.value || 'low',
...
function applyCalcState(state) {
  ...
  const dm = document.getElementById('calcDigMaturity');
  if (dm && state.digMaturity) dm.value = state.digMaturity;
