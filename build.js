...
import { buildFullKnowledge } from './web/extract-knowledge.js';
...
async function build() {
  ...
  // 3. Read knowledge base (markdown for the webpage)
  const knowledge = readFileSync(KB, 'utf-8');

  // 4. Extract FULL knowledge from all documents (for AI chat)
  console.log('Teljes tudásbázis kinyerése...');
  const { fullText, docCount, totalChars } = await buildFullKnowledge();

  // 5. Save full knowledge as JSON for netlify function
  writeFileSync(join(DIST, 'full-knowledge.json'), JSON.stringify({ fullText }));
...
window.__DOCS__ = ${JSON.stringify(docs)};
  ...
  console.log(`  - Knowledge base: ${knowledge.length} chars (webpage)`);
  console.log(`  - Full knowledge: ${docCount} docs, ${totalChars.toLocaleString()} chars (AI chat)`);
